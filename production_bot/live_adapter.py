...
class ProductionBinanceAdapter:
    ...
    The adapter accepts only AuthorizedOrder. Strategy/risk code never receives
    exchange credentials. Real-money execution is fail-closed by default.
    """
    ...
    def _check(self, order: AuthorizedOrder) -> None:
        ...
        if self.config.testnet is False and self.config.live_trading_enabled is not True:
            raise LiveExecutionBlocked("LIVE_TRADING_DISABLED")
